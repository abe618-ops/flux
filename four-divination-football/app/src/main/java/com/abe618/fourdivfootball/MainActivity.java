...
public class MainActivity extends Activity {
    ...
    private TextView directionView, primaryScoreView, resultDetailView, structureView, seedView;
    ...
    @Override public void onCreate(Bundle b) {
        super.onCreate(b);
        prefs = getSharedPreferences("four_divination_lab", MODE_PRIVATE);
        buildUi();
        recastRandom();
        refreshFrozen();
        refreshHistory();
    ...
    private void buildUi() {
        ...
        structureView.setGravity(Gravity.CENTER_HORIZONTAL);
        ...
        castCard.addView(text("起盘", 20, INK, true));
        labelInput = new EditText(this);
        labelInput.setHint("样本名称/比赛名称（可空）");
        labelInput.setSingleLine(true);
        castCard.addView(labelInput, new LinearLayout.LayoutParams(-1, dp(50)));
        seedInput = new EditText(this);
        seedInput.setHint("固定 Seed：输入数字或文字可复现");
        seedInput.setSingleLine(true);
        castCard.addView(seedInput, new LinearLayout.LayoutParams(-1, dp(50)));
        ...
        castCard.addView(freeze, new LinearLayout.LayoutParams(-1, dp(52)));
        seedView = text("", 12, MUTED, false);
        seedView.setPadding(0, dp(6), 0, 0);
        castCard.addView(seedView);
        root.addView(castCard);
        gap(12);

        root.addView(text("四术独立起盘", 20, INK, true));
        ...
        review.addView(record, new LinearLayout.LayoutParams(-1, dp(52)));
        root.addView(review);
        gap(12);
        ...
        history.addView(clear, new LinearLayout.LayoutParams(-1, dp(48)));
        root.addView(history);
        gap(12);

        LinearLayout note = card(Color.rgb(250,250,250));
        note.addView(text("模型说明", 17, INK, true));
        note.addView(text("四套体系独立随机起盘，合参层等权汇总。已纳入当前盲测经验：沙占零封信号单列；Ifá高度集中且沙占前静后强时开启3:0/0:3爆发分支；Sikidy稳定/双数象不再自动等同于小球。每场冻结后只允许录入赛果，不回改原盘。该工具用于盲测和统计实验，术数与足球赛果之间没有已建立的科学预测关系；“合参倾向度”不是现实概率。", 13, MUTED, false));
        root.addView(note);
    }

    private TextView systemCard(String title) {
        LinearLayout c = card(SOFT);
        TextView h = text(title, 17, BLUE, true);
        c.addView(h);
    ...
    private void recastFixed() {
        ...
        catch (Exception e) { seed = FourDivinationEngine.seedFromText(s); }
    ...
    private void showState() {
        ...
        directionView.setText(r.direction + "  ·  " + r.safer);
        primaryScoreView.setText("首选 " + r.primaryScore);
        resultDetailView.setText(
                "比分  " + r.scoreCandidates
                + "\n半全场  " + r.halfFull
                + "\n进球  " + r.goalRange + "（中心 " + r.totalGoals + "）"
                + "\n" + r.overUnder + "  ·  " + r.oddEven
                + (r.cleanSheet ? "  ·  零封分支" : "")
                + (r.burstBranch ? "  ·  爆发分支" : "")
        );
        structureView.setText("合参倾向度 " + r.tendency + "/100（非概率）\n" + r.structure);
        seedView.setText("本局 Seed：" + state.seed + " · 可复制复现");
    ...
    private void refreshFrozen() {
        if (!prefs.getBoolean("has_frozen", false)) {
            frozenView.setText("当前没有冻结样本。先起盘，再点击“冻结本场样本”。");
            return;
        }
        ...
        frozenView.setText("已冻结：" + label + "\n" + s.combined.direction + " · " + s.combined.scoreCandidates + " · Seed " + seed);
    }
    ...
    private void recordResult() {
        ...
        boolean hitOU = r.overUnder.equals(actualOU);
        boolean hitOE = r.oddEven.equals(actualOE);
        boolean hitScore = r.scoreCandidates.contains(actualScore);
        boolean hitRange = Math.abs(total - r.totalGoals) <= 1;
