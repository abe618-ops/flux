...
public final class FootballData {
    ...
    public static ArrayList<JSONObject> beidan(String html,String k)throws JSONException{
        ...
        while(tr.find()){
            ...
            Matcher odds=Pattern.compile("(?is)<span[^>]*class=['\"][^'\"]*\\bpjoz\\b[^'\"]*['\"][^>]*>\\s*([0-9.]+)\\s*</span>").matcher(tr.group(2));JSONObject p=new JSONObject();int n=0;while(odds.find()&&n<3){try{double v=Double.parseDouble(odds.group(1));if(validOdd(v))p.put(labels("HAD")[n],v);}catch(Exception ignored){}n++;}if(p.length()==3)prices(f,"HAD",p,"500 · 百家平均欧赔参考（不让球）");out.add(f);
        }return out;
    }
    ...
    public static boolean sameFixture(JSONObject a,JSONObject b){
        ...
        for(String k:new String[]{"jcId","titanId","fiveId"}){String v=a.optString(k);if(!v.isEmpty()&&v.equals(b.optString(k))){
            if(norm(a.optString("home")).equals(norm(b.optString("away")))&&!norm(a.optString("home")).equals(norm(b.optString("home"))))return false;
            return true;
        }}
        String ah=norm(a.optString("home")),aa=norm(a.optString("away")),bh=norm(b.optString("home")),ba=norm(b.optString("away"));long at=a.optLong("kickoff"),bt=b.optLong("kickoff");boolean near=at>0&&bt>0&&Math.abs(at-bt)<=20*60000;
        if(!ah.isEmpty()&&!aa.isEmpty()&&ah.equals(bh)&&aa.equals(ba)&&(near||at==0||bt==0))return true;
        return a.optInt("mode",-1)==0&&b.optInt("mode",-1)==0&&near&&!a.optString("number").isEmpty()&&a.optString("number").equals(b.optString("number"))&&!(ah.equals(ba)&&aa.equals(bh));
    }
