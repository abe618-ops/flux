...
public final class DataClient {
    ...
    public static JSONObject parseEuro(String js,long kickoff)throws JSONException{
        ...
        String[] ids={"","",""},names={"12BET","Bet365","澳门"};
        ...
        if(games.find()){Matcher item=Pattern.compile("\"([^\"]*)\"").matcher(games.group(1));while(item.find()){
            ...
            int ix=name.equals("12bet")?0:name.equals("bet365")?1:name.equals("macauslot")?2:-1;if(ix>=0)ids[ix]=cols[1];
        }}
        for(int p=0;p<ids.length;p++)for(String item:items){if(ids[p].isEmpty()||!item.startsWith(ids[p]+"^"))continue;long latest=Long.MIN_VALUE;JSONObject best=null;
