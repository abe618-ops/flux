...
public final class TicketStore {
    ...
    public TicketStore(Context c){prefs=c.getApplicationContext().getSharedPreferences("virtual_bet",0);migrate();}
    ...
    private void migrate(){synchronized(LOCK){if(prefs.getBoolean("schema_130",false))return;try{
        ...
        for(int i=0;i<tickets.length();i++){
            JSONObject t=tickets.getJSONObject(i);JSONArray ss=t.optJSONArray("selections");if(ss==null)continue;t.put("schema",3);int mode=t.optString("mode").contains("北")?1:t.optString("mode").contains("全球")?2:0;
    ...
    public ArrayList<JSONObject> cache(int mode,String date){ArrayList<JSONObject> out=new ArrayList<>();try{JSONArray a=new JSONArray(prefs.getString("fixtures130_"+mode+"_"+date,"[]"));for(int i=0;i<a.length();i++)out.add(a.getJSONObject(i));}catch(Exception ignored){}return out;}
    public void cache(int mode,String date,List<JSONObject> data){JSONArray a=new JSONArray();for(JSONObject f:data)a.put(f);prefs.edit().putString("fixtures130_"+mode+"_"+date,a.toString()).putLong("fixtures130_time_"+mode+"_"+date,System.currentTimeMillis()).apply();}
    ...
    public String sync(DataClient.Scope scope,Progress progress){
        ...
        try{
            ...
            for(String day:days){int mode=Integer.parseInt(day.substring(0,1));String d=day.substring(2);if(requests>=30)break;
                ArrayList<JSONObject> fromCache=new ArrayList<>();for(JSONObject f:cache(mode,d)){JSONObject r=f.optJSONObject("result");if(r!=null)fromCache.add(r);}if(!fromCache.isEmpty())applyResults(fromCache);
                for(int source=0;source<(mode==0?3:1);source++){final int s=source;service.submit(()->DataClient.results(d,mode,s,scope));requests++;}
            }
