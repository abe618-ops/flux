...
function runOuHybrid(maxAttempts=100000){
 const weighted=weightedOuFirstDraw();
 const ids=['zhouyi_ce','small_liuren','cegui_position'];
 for(let attempt=1;attempt<=maxAttempts;attempt++){
   const seed=freshSeed('ou-pair-consensus',attempt),by=castAll(seed);
   const a=by[ids[0]].ou,b=by[ids[1]].ou;
   if(a!==b)continue;
   const c=by[ids[2]].ou,agreeCount=c===a?3:2;
   const consensusBig=a===1?agreeCount/3:(3-agreeCount)/3;
   const finalBig=(consensusBig+weighted.bigShare)/2;
   const value=finalBig>=.5?1:0;
   return{
     dim:'ou',label:'大小',value,text:STAGES.ou.text(value),attempts:attempt,seed,
     agreement:agreeCount,
     consensusValue:a,consensusText:STAGES.ou.text(a),
     consensusBigShare:consensusBig,
     weighted,
     finalBigShare:finalBig,
     methods:ids.map(id=>({id,name:by[id].name,value:by[id].ou,text:STAGES.ou.text(by[id].ou),codes:by[id].codes,total:by[id].value}))
   };
 }
 throw new Error('大小项未在限定次数内形成前两项一致');
}
...
function makeRound(){return{id:'T'+String((state.history?.length||0)+1).padStart(4,'0'),createdAt:Date.now(),result:runStage('result'),ou:runOuHybrid(),parity:runStage('parity')}}function ensureRound(){if(current)return;current=makeRound();save()}
function rerun(dim){ensureRound();current[dim]=dim==='ou'?runOuHybrid():runStage(dim);save();renderCast();toast(dim==='ou'?'大小已按“两项/三项一致 + 首抽全权重”重新计算':STAGES[dim].label+'已重新抽到三者一致')}
...
function renderCast(){
 ...
 if(active==='ou'){
   const w=s.weighted;
   detail='<section class="card"><h2>大小综合依据</h2>'+
     '<div class="ou-grid">'+
       '<div class="ou-box"><span>前两项自动同向</span><b>'+esc(s.consensusText)+'</b><small>抽 '+s.attempts+' 次 · '+(s.agreement===3?'三项一致':'两项一致')+'</small></div>'+
       '<div class="ou-box"><span>首抽全权重</span><b>'+esc(w.text)+'</b><small>小 '+(w.smallShare*100).toFixed(1)+'% / 大 '+(w.bigShare*100).toFixed(1)+'%</small></div>'+
       '<div class="ou-box strong"><span>综合结果</span><b>'+esc(s.text)+'</b><small>综合大球值 '+(s.finalBigShare*100).toFixed(1)+'%</small></div>'+
     '</div>'+
     '<div class="tiny ou-note">前两项：周易策数 + 小六壬。二者不同就继续自动抽；一致后再看策轨元会运世，形成“两项一致”或“三项一致”。“首抽全权重”只使用第一次抽取的15种方法，不重复抽。</div>'+
   '</section>';
 }
 $('#app').innerHTML='<section class="hero"><div class="stage-tabs">'+buttons+'</div><div class="result-box"><div class="eyebrow">'+esc(cfg.label)+' · '+(active==='ou'?'两项/三项一致 + 全权重综合':'三者共同指向')+'</div><div class="result">'+esc(s.text)+'</div><div class="attempts">'+(active==='ou'?('前两项自动抽取 '+s.attempts+' 次达成同向，'+(s.agreement===3?'第三项也同向':'第三项反向')):('自动抽取 '+s.attempts+' 次后达成三者一致'))+'</div><div class="consensus">'+s.methods.map(m=>'<span>'+esc(m.name)+' → <b>'+esc(m.text)+'</b></span>').join('')+'</div></div></section>'+
 '<section class="card methods"><h2>'+(active==='ou'?'本次一致层':'本次共同指向')+'</h2>'+s.methods.map(m=>'<div class="method-row"><b>'+esc(m.name)+'</b><span class="dir">'+esc(m.text)+'</span></div>').join('')+'<button id="rerun" class="wide">重新抽取“'+esc(cfg.label)+'”</button></section>'+
 detail+
 '<section class="card"><h2>本轮三项结果</h2><div class="summary">'+summary+'</div><button id="newRound" class="wide ghost">开始新一轮</button></section>';
...
function renderRules(){$('#app').innerHTML='<section class="card"><h2>三轮测试法</h2><div class="rule-line"><b>① 胜负</b><div class="tiny">灵棋计数 + 策轨数 + 三位数梅花。三者方向不同就继续自动抽，直到三者同时指向主胜或同时指向客胜。</div></div><div class="rule-line"><b>② 大小</b><div class="tiny">先让周易策数 + 小六壬自动重抽到两项一致，再看策轨元会运世是否形成三项一致；同时保留第一次抽取的15种方法，按原大小权重合计“小/大”票。最终把一致层与首抽全权重等权综合，避免单一方法把结果锁死为大球。</div></div><div class="rule-line"><b>③ 单双</b><div class="tiny">梅花卦数 + 策轨元会运世实验 + 三位数梅花。三者一致才显示双或单。</div></div><p class="tiny">三个项目彼此独立抽取，不使用V1.3旧权重，也不再使用“奇门+六爻前2同向”作为门控。</p></section>'}
