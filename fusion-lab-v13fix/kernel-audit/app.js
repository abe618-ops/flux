...
const PRIOR_N=8;let tab='cast';let current=null;
let state=load();
...
const DISPLAY_ORDER=['qimen','liuyao','meihua','taixuan','cegui_position','jinkou','cegui','zhouyi_ce','daliuren','lingqi','lingqi_classic','wuxing','small_liuren','shenyi','meihua_canonical'];
function ordered(){const pos=Object.fromEntries(DISPLAY_ORDER.map((id,i)=>[id,i]));return [...METHODS].sort((a,b)=>pos[a.id]-pos[b.id])}
...
function makeForecast(){const top=ordered().slice(0,2).map(x=>x.id);let attempts=0,methods,seed,aligned=false;do{attempts++;const a=new Uint32Array(4);crypto.getRandomValues(a);seed=[...a].join('-')+'-'+attempts;methods=METHODS.map(m=>castMethod(m,seed));const by=Object.fromEntries(methods.map(x=>[x.id,x]));aligned=by[top[0]].result===by[top[1]].result}while(!aligned);const result=advice(methods,'result'),ou=advice(methods,'ou'),parity=advice(methods,'parity');return {id:'R'+String(state.records.length+1).padStart(4,'0'),createdAt:Date.now(),seed,attempts,top2:top,methods,result:result.winner.dir,ou:ou.winner.dir,parity:parity.winner.dir,detail:{result,ou,parity},actual:null}}
...
function renderCast(){const rank=ordered();if(!current){$('#app').innerHTML=`<section class="card"><h2>新一场</h2><input id="label" placeholder="场次标签，可留空"><button id="go" class="wide">开始自动合参</button><p class="tiny">按指定优先级排序；奇门拆补盘与六爻纳甲若主客方向不同就自动重抽，直到一致。单双由每个术数独立取数，再进行总体加权合参。</p></section><section class="card"><h2>当前前两项</h2><div class="rank"><b>#1 ${esc(rank[0].name)}</b><span>${(composite(rank[0])*100).toFixed(1)}%</span></div><div class="rank"><b>#2 ${esc(rank[1].name)}</b><span>${(composite(rank[1])*100).toFixed(1)}%</span></div></section>`;$('#go').onclick=()=>{current=makeForecast();current.label=$('#label').value.trim()||'盲测第'+(state.records.length+1)+'场';render()};return}
 const r=current,order=ordered().map(x=>x.id),sorted=[...r.methods].sort((a,b)=>order.indexOf(a.id)-order.indexOf(b.id)),counts={result:[0,0],ou:[0,0],parity:[0,0]};r.methods.forEach(x=>{counts.result[x.result]++;counts.ou[x.ou]++;counts.parity[x.parity]++});
 $('#app').innerHTML=`<section class="hero"><div class="row"><b>${esc(r.label)}</b><button id="next" class="ghost">下一场</button></div><div class="pick">${LABEL.result[r.result]} / ${LABEL.ou[r.ou]} / ${LABEL.parity[r.parity]}</div><div class="meta"><span>主${counts.result[0]} 客${counts.result[1]}</span><span>小${counts.ou[0]} 大${counts.ou[1]}</span><span>双${counts.parity[0]} 单${counts.parity[1]}</span><span>前2项 ${r.attempts} 抽同向</span></div></section><section class="card methods"><h2>方法排序 · 高 → 低</h2>${sorted.map((x,i)=>{const m=METHODS.find(z=>z.id===x.id),top=r.top2.includes(x.id);return `<div class="method ${top?'top':''}"><b>#${i+1} ${esc(x.name)}${top?'<i class="badge">前2</i>':''}</b><span class="picks">${LABEL.result[x.result]} / ${LABEL.ou[x.ou]} / ${LABEL.parity[x.parity]}</span><span class="score">${(composite(m)*100).toFixed(1)}%</span></div>`}).join('')}</section><section class="card"><div class="kpis"><div class="kpi"><span>胜负</span><b>${LABEL.result[r.result]}</b></div><div class="kpi"><span>大小</span><b>${LABEL.ou[r.ou]}</b></div><div class="kpi"><span>单双</span><b>${LABEL.parity[r.parity]}</b></div></div><button id="settle" class="wide">输入赛果复盘</button><details><summary>共振与单双取数</summary><p class="tiny">胜负/大小：单法历史权重82% + 达标共振18%。单双：各术数独立取数后按其历史单双成绩加权，并同样记录2–5家共振。</p>${sorted.map(x=>`<div class="rank"><span>${esc(x.name)} → <b>${LABEL.parity[x.parity]}</b></span><small>合数 ${x.value}<br>${esc(x.rule)}</small></div>`).join('')}</details></section>`;
...
function renderRules(){$('#app').innerHTML=`<section class="card"><h2>逐术数单双取数</h2><p class="tiny">以下是本软件用于盲测的“实验映射”，用于建立可复盘的独立单双样本，不宣称等同于某一古籍的唯一传统定法。</p>${ordered().map(m=>`<details><summary>${esc(m.name)}</summary><p class="tiny">${esc(RULES[m.id])}</p></details>`).join('')}</section><section class="card"><h2>合参规则</h2><p class="tiny">① 方法按本轮指定优先级排列；② 奇门拆补盘与六爻纳甲胜负不一致时自动重抽，直到一致；③ 单法权重为主体，达标的2–5家共振有限修正；④ 单双由15家各自取数后独立加权；⑤ 小样本2/2、3/3不会直接当成稳定规律。</p></section>`}
