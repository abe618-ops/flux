...
/**
 * 仅为本地虚拟赔率指数学习提供有界只读网络请求；不接入支付、充值、出票或真实投注。
 * 页面刷新与赛果同步使用独立 Scope，避免旧请求拖住界面。
 */
public final class DataClient {
    ...
    public static ArrayList<JSONObject> fixtures(int mode,int source,String k,Scope scope)throws Exception{
        ...
        if(mode==1){
            ...
            ArrayList<JSONObject> out=FootballData.globalTitan(get(url,"GB18030",scope),k);
            if(k.equals(FootballData.today())){
                try{out=FootballData.merge(out,FootballData.globalTitan(get(titanUrl(k),"GB18030",scope),k),false);}catch(Exception ignored){}
            }
            return out;
        }
        if(mode==2){if(source>0)return new ArrayList<>();return FootballData.globalTitan(get(url,"GB18030",scope),k);}
        return new ArrayList<>();
    }
    public static ArrayList<JSONObject> results(String k,int mode,int source,Scope scope)throws Exception{
        ArrayList<JSONObject> fixtures;if(mode==1||mode==2)fixtures=FootballData.globalTitan(get(titanUrl(k),"GB18030",scope),k);else fixtures=source==0?FootballData.titan(get(titanUrl(k),"GB18030",scope),k,mode):source==1?FootballData.sina(get(sinaUrl(k),"UTF-8",scope),k):FootballData.five(get(fiveUrl(k),"GB18030",scope),k);
    ...
    public static JSONObject euro(String id,long kickoff,Scope scope)throws Exception{return euro(id,kickoff,"自动",scope);}
    public static JSONObject euro(String id,long kickoff,String preferred,Scope scope)throws Exception{
        if(!id.matches("\\d+"))throw new IOException("缺球探赛程编号");
        return parseEuro(get("https://txt.titan007.com/1x2/"+id+".js","UTF-8",scope),kickoff,preferred);
    }
    static String providerName(String raw){
        String n=raw==null?"":raw.toLowerCase(Locale.ROOT).replace(" ","").replace("-","");
        if(n.contains("12bet"))return "12BET";
        if(n.contains("10bet"))return "10BET";
        if(n.contains("bet365"))return "Bet365";
        if(n.contains("crown")||n.contains("皇冠"))return "Crown";
        if(n.contains("macau")||n.contains("澳门"))return "澳门";
        return "";
    }
    public static JSONObject parseEuro(String js,long kickoff)throws JSONException{return parseEuro(js,kickoff,"自动");}
    public static JSONObject parseEuro(String js,long kickoff,String preferred)throws JSONException{
        ...
        if(games.find()){Matcher item=Pattern.compile("\"([^\"]*)\"").matcher(games.group(1));while(item.find()){
            String[] cols=item.group(1).split("\\|",-1);if(cols.length<3)continue;String provider=providerName(cols[2]);if(!provider.isEmpty()&&!cols[1].isEmpty())ids.put(provider,cols[1]);
        }}
        ArrayList<String> order=new ArrayList<>();
        if(preferred!=null&&!preferred.isEmpty()&&!"自动".equals(preferred))order.add(preferred);
        else order.addAll(Arrays.asList("12BET","10BET","Bet365","Crown","澳门"));
        for(String provider:order){
            String companyId=ids.get(provider);if(companyId==null||companyId.isEmpty())continue;
            long latest=Long.MIN_VALUE;JSONObject best=null;
            for(String item:items){if(!item.startsWith(companyId+"^"))continue;
                for(String row:item.substring(item.indexOf('^')+1).split(";")){
                    String[] fields=row.split("\\|",-1);if(fields.length<4)continue;try{
                        ...
                        if(at>=latest){latest=at;JSONObject prices=new JSONObject();prices.put("胜",h);prices.put("平",d);prices.put("负",a);best=new JSONObject();best.put("prices",prices);best.put("source",provider+" · 浮动欧赔");best.put("provider",provider);best.put("at",at);}
