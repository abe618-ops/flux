...
public final class FootballData {
    ...
    public static boolean sameFixture(JSONObject a,JSONObject b){
        ...
        boolean teams=teamSimilar(a.optString("home"),b.optString("home"))&&teamSimilar(a.optString("away"),b.optString("away"));
        if(teams)return true;
        long at=a.optLong("kickoff"),bt=b.optLong("kickoff");boolean near=at>0&&bt>0&&Math.abs(at-bt)<=180*60000;
        int am=a.optInt("mode",-1),bm=b.optInt("mode",-1);String an=a.optString("number"),bn=b.optString("number");
        if(am==1&&near&&!an.isEmpty()&&an.equals(bn))return true;
    ...
    public static ArrayList<JSONObject> attachRelevant(List<JSONObject> base,List<JSONObject> candidates)throws JSONException{
        ArrayList<JSONObject> out=new ArrayList<>();for(JSONObject x:base)out.add(copy(x));
        for(JSONObject x:candidates){
            JSONObject match=null;int hits=0;
            for(JSONObject y:out)if(sameFixture(y,x)){match=y;hits++;}
            if(hits!=1||match==null)continue;
            for(String key:new String[]{"titanId","fiveId","jcId","sinaId"})if(!x.optString(key).isEmpty())match.put(key,x.optString(key));
            if(match.optString("league").isEmpty()&&!x.optString("league").isEmpty())match.put("league",x.optString("league"));
            if(match.optLong("kickoff",0)<=0&&x.optLong("kickoff",0)>0)match.put("kickoff",x.optLong("kickoff"));
            JSONObject r=x.optJSONObject("result");if(r!=null)match.put("result",copy(r));
        }
        return out;
    }
