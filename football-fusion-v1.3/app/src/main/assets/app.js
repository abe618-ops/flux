...
function renderCast(){const rank=ordered();if(!current){$('#app').innerHTML=`<section class="card"><h2>新一场</h2><input id="label" placeholder="场次标签，可留空"><button id="go" class="wide">开始自动合参</button><p class="tiny">按当前准确率排序；排名前两项若主客方向不同就自动重抽，直到一致。单双由每个术数独立取数，再进行总体加权合参。</p></section><section class="card"><h2>当前前两项</h2><div class="rank"><b>#1 ${esc(rank[0].name)}</b><span>${(composite(rank[0])*100).toFixed(1)}%</span></div><div class="rank"><b>#2 ${esc(rank[1].name)}</b><span>${(composite(rank[1])*100).toFixed(1)}%</span></div></section>`;$('#go').onclick=()=>{current=makeForecast();current.label=$('#label').value.trim()||'盲测第'+(state.records.length+1)+'场';render()};return}
